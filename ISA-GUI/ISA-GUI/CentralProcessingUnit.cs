	...
	*/
    internal class CentralProcessingUnit
    {
	    ...
	    */
        ...
        {
            int opcode, r1, r2, r3, address;            //parameters for the instructions
            string instrType;
            ...
            if (debug && IM.instructions.Count == 0)    //if in debug mode and the program hasn't been started
            {
                ...
                fetch.getNextInstruction(ref registers, ref IM);        //FETCH - get the next instruction
                CU.decode(ref IM, out opcode, out r1, out r2, out r3, out address, out instrType);      //DECODE - Decode the instruction
                ...
                buildDecodedString(ref decodedString, opcode, r1, r2, r3, address, instrType);      //Build the decoded instruction string
            }
        ...
        *   @param  string instrType
        */
        public void buildDecodedString(ref StringBuilder decodedString, int opcode, int r1, int r2, int r3, int address, string instrType)
        {
            ...
            if (opcode == 0 || opcode == 6 || (opcode >= 8 && opcode <=15))
            {
                decodedString.Append((string.Format("\n{0, 7} {1, 4} {2, 8} {3, 9} {4, 4} {5, 4} {6, 4} {7, 8}",
                            (IM.ProgramCounter - 2).ToString("X").PadLeft(4, '0'), opcode.ToString("X"), instructions[opcode], instrType,
